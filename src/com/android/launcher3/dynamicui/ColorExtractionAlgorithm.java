...
public class ColorExtractionAlgorithm
{
    ...
    public static ColorExtractionAlgorithm newInstance(final Context context) {
        return Utilities.getOverrideObject(ColorExtractionAlgorithm.class, context.getApplicationContext(), 2131492892);
    }
    ...
    public Pair extractInto(final WallpaperColorsCompat wallpaperColorsCompat) {
        ...
        final List<Integer> mainColors = getMainColors(wallpaperColorsCompat);
        ...
        while (true) {
            while (i < size) {
                ...
                ColorUtils.RGBToHSL(Color.red(intValue), Color.green(intValue), Color.blue(intValue), array);
                if (!this.isBlacklisted(array)) {
                    ...
                    ColorUtils.RGBToHSL(Color.red(intValue2), Color.green(intValue2), Color.blue(intValue2), array);
                    array[0] /= 360.0f;
                    ...
                    ColorUtils.colorToHSL(-0x4F4F50, this.mTmpHSL);
                    if (n4 > this.mTmpHSL[n2]) {
                        return applyFallback(wallpaperColorsCompat);
                    }
                    ColorUtils.colorToHSL(-0xDEDEDF, this.mTmpHSL);
                    if (n4 < this.mTmpHSL[n2]) {
                        return applyFallback(wallpaperColorsCompat);
                    }
                    if (n3 != 0) {
                    ...
                    return new Pair(colorInt, this.getColorInt(n5 + min, fit, fit2, fit3));
                }
                else {
                    ++i;
                }
